...
def readDepthVKITTI(filename):
    """
    Calcula el mapa de profundidad a partir del mapa de disparidad utilizando los parámetros de la cámara.

    Args:
        filename (str): Ruta del archivo de la imagen de disparidad.

    Returns:
        tuple: (depth, valid) donde `depth` es el mapa de profundidad y `valid` es un mapa booleano indicando los valores válidos.
    """
    disparity = cv2.imread(filename, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    f = 725.0087  # Distancia focal en píxeles
    baseline = 0.532725  # Distancia entre cámaras en metros

    # Evitar la división por cero en la disparidad
    disparity[disparity == 0] = 1e-6

    # Calcular la profundidad (depth) a partir de la disparidad
    depth = f * baseline / disparity  # Profundidad
